...
# Expand matrix to get a 48bits matrix of datas to apply the xor with Key
# R32 to 48 bits
expandPermutation = [
    32, 1, 2, 3, 4, 5,
    4, 5, 6, 7, 8, 9,
...
# Parity-check bits(64 bits) to 56 bit
# because bits 8,16, 24,32,40,48,56,64 are not chosen in PC-1
PC1 = [
    57, 49, 41, 33, 25, 17, 9,
    1, 58, 50, 42, 34, 26, 18,
    10, 2, 59, 51, 43, 35, 27,
...
# PC-2 selects 48 bits from PC-1 56 bits
PC2 = [
...
# 48 bits to 32 bits
# S-box substitution has 8 blocks of 6-bit to 4-bit each of 8 substitution
Sboxes = [
    [
...
def DES(text, key, padding, isEncrypt):
    ...
    for block in plaintext8byteBlocks:
        ...
        for i in range(16):
            ...
            expandedRightBlock = expand(rightBlock, expandPermutation)
        ...
        result += permutation(rightBlock + leftBlock, finalPermutation)
